    ...
    try {
      const res = await fetch('/api/ingestion/parse-preview', {
        ...
        body: JSON.stringify({
          ...
          parent_bkc: active.parent_bkc,
          experiment_label: active.experiment_label,
          games: active.games,
        }),
      });
  ...
  return (
    <div className="flex flex-col h-full bg-[#140f2d]/60 border border-amber-500/25 rounded-xl overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Metadata Sidebar */}
        <div className="w-56 flex-shrink-0 border-r border-white/5 p-3 flex flex-col gap-2 overflow-y-auto">
          ...
          />
          <label className="text-[10px] text-slate-500 uppercase tracking-wider">SKU ID</label>
          <input
            type="text"
            value={active.sku_id}
            onChange={e => updateActive({ sku_id: e.target.value })}
            placeholder="e.g. nvl-s"
            className="px-2 py-1 rounded bg-white/5 border border-white/10 text-xs text-white placeholder-slate-600 outline-none focus:border-amber-500/50 w-full"
          />
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border cursor-pointer transition-all w-full ${
            ...
            <div className="border-t border-amber-500/20 bg-amber-500/[0.03] overflow-y-auto max-h-[160px]">
              ...
              {reviewResults.map((r, i) => (
                <div key={i} className="flex items-center px-3 py-1 gap-3 border-b border-white/[0.02] text-xs">
                  <span className="flex-1 text-slate-300 truncate">{r.game || r.name || '—'}</span>
                  <span className="w-16 text-right text-white">{r.avg_fps ?? '—'}</span>
                  <span className="w-16 text-right text-white">{r.p1_low ?? r.low_1pct ?? '—'}</span>
                  <span className="w-14 text-right text-white">{r.power ?? '—'}</span>
                  <span className="w-14 text-right text-white">{r.temp ?? '—'}</span>
                  <span className="w-14 text-right text-white">{r.charts_count ?? r.charts ?? '—'}</span>
            ...
            <div className={`px-3 py-2 border-t text-xs flex items-center gap-2 ${
              ...
              {pushResult.error ? (
                ...
                <>
                  <CheckCircle size={14} />
                  <span>
                    Successfully pushed {pushResult.count ?? pushResult.games_count ?? '?'} games.
